/**
 * Country data with ISO 3166-1 alpha-2 codes for flag rendering.
 * Shared between SIMs.jsx, PhoneInput.jsx, and CountryCodeSelect — do not duplicate.
 *
 * Flag images are served from flagcdn.com (free, no API key needed).
 * Windows does not render Unicode flag emojis, so we use PNG images instead.
 */

// Returns flag image URL from flagcdn.com for a given ISO code
// e.g., getFlagUrl('IN') → 'https://flagcdn.com/w20/in.png'
export function getFlagUrl(iso, width = 20) {
  if (!iso || iso.length !== 2) return ''
  return `https://flagcdn.com/w${width}/${iso.toLowerCase()}.png`
}

// Derive flag URL from a phone number string (e.g., '+919876543210' → India flag URL)
export function getFlagFromPhone(phone, width = 20) {
  if (!phone || !phone.startsWith('+')) return ''
  const match = countryCodeMap[getDialCode(phone)]
  return match ? getFlagUrl(match.iso, width) : ''
}

// Extract the matching dial code from a phone number (longest match first)
export function getDialCode(phone) {
  if (!phone || !phone.startsWith('+')) return ''
...
// Get country data from a phone number
export function getCountryFromPhone(phone) {
  if (!phone || !phone.startsWith('+')) return null
  return countryCodeMap[getDialCode(phone)] || null
}
...
// Fast lookup: dial code → { code, country, iso }
export const countryCodeMap = countryCodes.reduce((map, c) => {
  if (!map[c.code]) map[c.code] = c
  return map
}, {})